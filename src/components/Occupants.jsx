...
function Occupants({ searchQuery, setSearchQuery, openCreateModalToken = null }) {
  ...
  const handleDeleteOccupant = async (occupant) => {
    if (!occupant?.fromBackend) {
      setOccupantMessage('❌ Suppression uniquement pour les occupants du backend.')
      return
    }
    if (!window.confirm(`Supprimer ${occupant.lastName} ${occupant.firstName} ?`)) return
    setOccupantActionId(occupant.id)
    setOccupantMessage('')
    try {
      await deleteUser(occupant.id)
      await handleRefreshOccupants()
      setOccupantMessage('✅ Occupant supprimé')
    } catch (error) {
      setOccupantMessage(`❌ Erreur suppression: ${error?.message || 'Erreur inconnue'}`)
    } finally {
      setOccupantActionId(null)
    }
  }
